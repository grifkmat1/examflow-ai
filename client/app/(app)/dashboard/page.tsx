...
const DEMO: Exam[] = [
  ...
  { id: '3', course_code: 'PHYS201', course_title: 'Mechanics', exam_type: 'FINAL', start_time: '2025-05-14T14:00:00Z', end_time: '2025-05-14T16:00:00Z', semester: 'Spring 2025', location: 'Lab C', credit_hours: 4 },
]
const DEMO_CONFLICTS: Conflict[] = [
  { type: 'OVERLAP', severity: 'HIGH', message: 'CS401 and MATH301 overlap on May 12th.', exams: [DEMO[0], DEMO[1]] }
]
const QUICK = [
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-2">
          {isDemo && <span className="text-[10px] bg-amber-500/15 text-amber-400 border border-amber-500/20 px-2.5 py-1 rounded-full">Demo data</span>}
          <span className={`text-[10px] px-2.5 py-1 rounded-full border ${apiStatus === 'online' ? 'bg-emerald-500/15 text-emerald-400 border-emerald-500/20' : apiStatus === 'offline' ? 'bg-red-500/10 text-red-400 border-red-500/20' : 'bg-white/5 text-white/30 border-white/10'}`}>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard label="Finals" value={finals} sub="high-stakes" accent="text-orange-400" icon="🎓" />
        </div>
      )}
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <Link href="/exams" className="text-xs text-emerald-400 hover:text-emerald-300 font-medium">View all →</Link>
        </div>
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">{[1,2,3].map(i => <CardSkeleton key={i} />)}</div>
        ) : exams.length === 0 ? (
          <Card>
            <EmptyState icon="📅" title="No exams yet" desc="Add your first exam to start tracking." action={<Link href="/exams" className="text-xs bg-emerald-500 text-black font-semibold px-4 py-1.5 rounded-lg">Add exam</Link>} />
          </Card>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {exams.slice(0, 6).map(exam => (
              <Card key={exam.id} hover className="p-4 group">
                ...
                <h3 className="text-sm font-semibold text-white/85 mb-2.5 line-clamp-1">{exam.course_title}</h3>
                <p className="text-xs text-white/40">{format(new Date(exam.start_time), 'MMM d, yyyy · h:mm a')}</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {QUICK.map(a => (
            <Link key={a.href} href={a.href} className="group p-4 bg-gray-900 border border-white/8 rounded-xl hover:border-white/15 hover:bg-gray-800/80 transition-all">
              ...
              <p className="text-sm font-semibold text-white/80 group-hover:text-white">{a.label}</p>
